...
<head>
	...
	<script src="http://<?php echo $_SERVER['HTTP_HOST']; ?>/assets/js/tour.js"></script>
</head>
<body>
...
<div id="pano" style="width: 100%; height: 100%;">
	...
	<script type="text/javascript">
		embedpano({
			swf: "http://<?php echo $_SERVER['HTTP_HOST']; ?>/assets/krpano/tour.swf",
			xml: "http://<?php echo $_SERVER['HTTP_HOST']; ?>/_/<?php echo $uId; ?>/vtour/tour.xml?" + Math.round(Math.random() * 1000000000).toString(),
			target: "pano",
			html5: "prefer",
		...
		$.getJSON('http://<?php echo $_SERVER['HTTP_HOST']; ?>/_/<?php echo $uId; ?>/data.json?' + Math.random(), function (JSONdata) {
			data = JSONdata;
			console.info(data);
...
// 				document.getElementById('set_defaultView').disabled = true;
			document.getElementById('moveHotspot').disabled = true;
...
// 				document.getElementById('set_defaultView').disabled = false;
		...
		function hmv(currentHotspot, currentScene, i) {
			console.log('is is: ' + i);
			console.info(currentHotspot);
// 				console.log( typeof currentHotspot );
...
// 				hotspotList[sceneName][current_randome_val + current_vTour_hotspot_counter.toString()] = currentHotspotData;
			...
			console.log('hmv: url: ' + currentHotspot.url);

			//if hotspot just live in js var ( not live in xml yet )
			if (currentHotspot.url == 'hotspot.png') {
			...
			else // it live in xml, and will auto-reload-by krpano, so we need to
			{
				...
				console.log('hmv: xreRender: [' + i + '] ' + krpano.get("hotspot[" + i + "].xreRender"));
			}
		}
		...
		function superHotspotObj(krpano_Obj) {
			...
			this.saveCurrentHotspotFromCurrentScene = function () {
				...
				for (var i = 0; i < hotspot_count; i++) {
					if (thisAlias.kr.get('hotspot[' + i + '].url') == 'hotspot.png' || thisAlias.kr.get('hotspot[' + i + '].url') == atob("aHR0cDovL2RhdGEuZ2xvYmFsdmlzaW9uLmNoL2tycGFuby8xLjE5L3NraW4vdnRvdXJza2luX2hvdHNwb3QucG5n") || thisAlias.kr.get('hotspot[' + i + '].url') == atob("aHR0cDovL2RhdGEuZ2xvYmFsdmlzaW9uLmNoL2tycGFuby8xLjE5L3NraW4vaW5mb3JtYXRpb24ucG5n")) {
						...
						if (thisAlias.kr.get('hotspot[' + i + '].url') == atob("aHR0cDovL2RhdGEuZ2xvYmFsdmlzaW9uLmNoL2tycGFuby8xLjE5L3NraW4vdnRvdXJza2luX2hvdHNwb3QucG5n") || thisAlias.kr.get('hotspot[' + i + '].url') == atob("aHR0cDovL2RhdGEuZ2xvYmFsdmlzaW9uLmNoL2tycGFuby8xLjE5L3NraW4vaW5mb3JtYXRpb24ucG5n")) {
			...
			this.loadHotspotsToCurrentSceneFromSavedData = function () {
				...
				sceneName = this.kr.get('xml.scene');
				console.info('loadHotspotsToCurrentSceneFromSavedData: ' + sceneName);
				for (var hotspotId in thisAlias.hotspotList[sceneName]) {
					...
					if (thisAlias.hotspotList[sceneName][hotspotId].reRender == "true") {
						console.log('rdy for rerender: ');
						console.info(currentHotspotData);
						add_hotspot_to_scene(currentHotspotData);
					}
					else
						console.log('skip reRender: ' + hotspotId);
				}
